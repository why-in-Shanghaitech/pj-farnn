...
class TimeoutFunction:
    ...
    def __call__(self, *args, **keyArgs):
        # A thread wrapper for time out function

        def to_do(func, args, kwargs, result, error):
            try:
                result.append(func(*args, **kwargs))
            except BaseException as inst: # capture all exceptions
                error.append(inst)
        ...
        def wapper(*args, **kwargs):
            ...
            try:
                ...
                # Unknown error, make sure to kill the thread
                t.kill()
                raise
            
            if t.is_alive():
                t.kill()
                emsg="function(%s) execute timeout after %d second" % (self.function.__name__, self.timeout)
                raise TimeoutFunctionException(emsg)
            elif result:
                return result.pop()
            elif error:
                raise error.pop()

        return wapper(*args, **keyArgs)
