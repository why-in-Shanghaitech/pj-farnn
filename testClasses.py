...
class Question(object):
    ...
    def _execute(self, grades):
        timeout = self.questionDict.get('timeout', 1800) # a default timeout of half an hour
        return util.TimeoutFunction(self.execute, float(timeout))(grades)
...
class ExtraCreditPassAllTestsQuestion(Question):
    def __init__(self, questionDict):
        Question.__init__(self, questionDict)
...
class AccumulatedCreditQuestion(Question):
    """We define several testcase sets and get the point of the set if all testcases
    in the same set passed."""

    def execute(self, grades):
        assert 'testset' in self.questionDict, "Require config 'testset' for AccumulatedCreditQuestion."

        results = {testCase.name: f(grades) for testCase, f in self.testCases}
        for line in self.questionDict['testset'].split('\n'):
            score, tests = line.strip().split(':')
            if all(results.get(t.strip(), False) for t in tests.split(',')):
                grades.addPoints(int(score))
...
class TestCase(object):
    ...
    def _execute(self, grades, moduleDict, solutionDict):
        timeout = self.testDict.get('timeout', 1800) # a default timeout of half an hour
        return util.TimeoutFunction(self.execute, float(timeout))(grades, moduleDict, solutionDict)
